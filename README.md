...
### Magic-link sign-in
No separate identity provider is needed. Users type their `@heritagelab.ca` email on `/signin`, receive a one-click sign-in link via Resend, and are signed in for 30 days (database-backed sessions). Domain enforcement happens server-side in `src/lib/allowlist.ts`.
